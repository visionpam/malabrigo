...
const refresh = () => { revalidatePath("/pagos"); revalidatePath("/ventas"); revalidatePath("/socios"); revalidatePath("/"); };
...
export async function createPaymentAction(_state: PaymentActionState, formData: FormData): Promise<PaymentActionState> {
  ...
  try {
    await db.$transaction(async (transaction) => {
      const payment = await transaction.payment.create({ data: { ...parsed.data, reference: parsed.data.reference.toUpperCase(), currency: sale.currency, paidAt: new Date(`${parsed.data.paidAt}T12:00:00-05:00`) } });
      await reconcileInstallments(transaction, sale.id);
      await reconcileSaleReservation(transaction, sale.id);
      await transaction.auditLog.create({ data: { action: "PAYMENT_CREATED", entityType: "Payment", entityId: payment.id, after: { saleId: sale.id, reference: payment.reference, amount: payment.amount.toString(), concept: payment.concept, paidAt: payment.paidAt.toISOString(), status: payment.status } } });
    });
  } catch (error) {
