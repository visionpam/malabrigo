...
import { SaleDraftActions } from "./sale-draft-actions";
...
export default async function SalesPage() {
  ...
  return <><PageHeader eyebrow="Operación comercial" title="Ventas" description="Separaciones, contratos de programa y modalidades de pago." action={<SaleCreateDialog members={memberOptions} programs={programOptions} />} />
    ...
    <section className="module-panel"><div className="module-toolbar"><div><h2>Registro de ventas</h2><p>Los borradores se pueden editar o eliminar antes de confirmar la separación.</p></div></div>{sales.length === 0 ? <EmptyState icon={ShoppingBag} title="No hay ventas registradas" description="Las nuevas separaciones y contratos aparecerán en este módulo." /> : <div className="table-wrap module-table"><table><thead><tr><th>Código</th><th>Socio</th><th>Programa</th><th>Modalidad</th><th>Valor</th><th>Estado</th><th>Fecha</th><th></th></tr></thead><tbody>{sales.map((sale) => <tr key={sale.id}><td className="money">{sale.code}</td><td>{sale.member.firstName} {sale.member.lastName}</td><td>{sale.program.name}</td><td>{sale.mode === "CASH" ? "Contado" : `${sale.financingPlan?.termMonths ?? "—"} meses`}</td><td className="money">{formatMoney(sale.totalPrice)}</td><td><StatusPill tone={statusTone(sale.status)}>{saleStatusLabel[sale.status]}</StatusPill></td><td>{formatDate(sale.createdAt)}</td><td><div className="sale-table-actions"><SaleManageDialog sale={{ id: sale.id, code: sale.code, member: `${sale.member.firstName} ${sale.member.lastName}`, program: sale.program.name, mode: sale.mode, termMonths: sale.financingPlan?.termMonths ?? null, status: sale.status, totalPrice: Number(sale.totalPrice), separationAmount: Number(sale.separationAmount), downPaymentAmount: Number(sale.downPaymentAmount), confirmedPayments: sale.payments.reduce((sum, payment) => sum + Number(payment.amount), 0), signedAt: sale.signedAt ? formatDate(sale.signedAt) : null, installments: sale.installments.map((item) => ({ number: item.number, amount: Number(item.amount), dueDate: formatDate(item.dueDate), status: item.status })), history: (historyBySale.get(sale.id) ?? []).map((item) => ({ id: item.id.toString(), action: item.action, createdAt: formatDate(item.createdAt) })) }} />{sale.status === "DRAFT" && <SaleDraftActions sale={{ id: sale.id, code: sale.code, memberId: sale.memberId, programId: sale.programId, mode: sale.mode, financingPlanId: sale.financingPlanId }} members={memberOptions} programs={programOptions} />}</div></td></tr>)}</tbody></table></div>}</section>
  </>;
}
