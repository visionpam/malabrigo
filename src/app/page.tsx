...
export default async function DashboardPage() {
  ...
  const [activeMembers, collectedMonth, pendingSubmissions, allocatedShares, yearlyPayments, recentSubmissions, stages, unsignedContracts] = await Promise.all([
    ...
    db.paymentSubmission.findMany({ take: 5, orderBy: { submittedAt: "desc" }, select: { id: true, concept: true, amount: true, submittedAt: true, status: true, voucherUrl: true, sale: { include: { member: true, program: true } } } }),
    db.projectStage.findMany({ where: { projectId: featuredProject?.id ?? "00000000-0000-0000-0000-000000000000" }, orderBy: { sortOrder: "asc" }, include: { updates: { orderBy: { occurredAt: "desc" }, take: 1 } } }),
  ...
  return <>
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel project-panel"><div className="panel-header"><div><span className="panel-kicker">{featuredProject?.name ?? "Proyecto"}</span><h2>Avance de obra</h2></div></div><div className="progress-ring" style={{ "--progress": `${Math.min(100, projectProgress)}%` } as React.CSSProperties}><div><strong>{projectProgress.toFixed(1)}%</strong><span>general</span></div></div><div className="stage-copy"><strong>{currentStage?.name ?? "Cronograma pendiente"}</strong><span>{currentStage?.description ?? "Configura las etapas oficiales del proyecto."}</span><small>{stages.length} etapas registradas</small></div><Link className="secondary-button" href={featuredProject ? `/avance-obra?proyecto=${featuredProject.id}` : "/proyectos"}><Upload size={16} /> Ver avance</Link></article>
