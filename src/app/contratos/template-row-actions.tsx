...
export function TemplateRowActions({ template }: { template: Template }) {
  ...
  return <><div className="row-actions"><button className="row-action row-action-icon" type="button" aria-label={`Ver plantilla ${template.name}`} title="Ver plantilla" onClick={() => setMode("view")}><Eye size={17} aria-hidden="true" /></button><button className="row-action row-action-icon edit-link" type="button" aria-label={`Editar plantilla ${template.name}`} title="Editar plantilla" onClick={() => setMode("edit")}><Pencil size={17} aria-hidden="true" /></button><button className="row-action row-action-icon danger-link" type="button" aria-label={`Eliminar plantilla ${template.name}`} onClick={() => setMode("delete")} disabled={template.used} title={template.used ? "No se puede eliminar una plantilla usada en contratos" : "Eliminar plantilla"}><Trash2 size={17} aria-hidden="true" /></button></div>{message && <span className="toast-message" role="status">{message}</span>}{mode === "view" && <TemplateModal title={`${template.name} · v${template.version}`} onClose={() => setMode(null)}><div className="member-form"><p><strong>Código:</strong> {template.code}</p><pre style={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere", fontFamily: "inherit" }}>{template.content}</pre><div className="dialog-actions"><button className="cancel-button" type="button" onClick={() => setMode(null)}>Cerrar</button></div></div></TemplateModal>}{mode === "edit" && <TemplateModal title={`Editar ${template.name}`} onClose={() => setMode(null)}><EditTemplate template={template} onDone={done} /></TemplateModal>}{mode === "delete" && <TemplateModal title="Eliminar plantilla" onClose={() => setMode(null)}><DeleteTemplate template={template} onDone={done} /></TemplateModal>}</>;
}
